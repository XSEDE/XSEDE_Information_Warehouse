...
class Resource_List(APIView):
    ...
    def get(self, request, format=None):
        pdb.set_trace()
        active_resourceids = RDR_Active_All_ResourceIDs()
...
class Resource_List_Active(APIView):
    permission_classes = (IsAuthenticatedOrReadOnly,)
    def get(self, request, format=None):
        objects = RDR_Active_Sub_Resources().order_by('info_resourceid')
        for o in objects:
            o.Active = True
        c = Context({'resource_list': objects})
        return render(request, 'resources.html', c)
...
class Software_XUP_v1_List(APIView):
    permission_classes = (IsAuthenticatedOrReadOnly,)
    def get(self, request, format=None):
        my_resourceids = Active_ResourceIDs()
        objects = ApplicationHandle.objects.filter(ResourceID__in=my_resourceids)
        serializer = Software_Full_Serializer(objects, many=True)
        return Response(serializer.data)
