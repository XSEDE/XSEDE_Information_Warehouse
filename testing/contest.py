...
class Resource:
	...
	def  multiTest(self, connection, iterations):
		...
		print ("\nTesting https://%s/%s for #%d iterations"% (connection.host,self.path,iterations))
			...
			try:
				...
				print ("\033[1;31mTest https://%s/%s failed on iteraton %d:  %s\033[0;0m"% (connection.host,self.path,test, str(e)))
...
class Test:
        ...
        def __init__(self):
                ...
                parser.add_option("-o", "--logfile", dest="logfile", help="Name of file to which output should be logged", default=None)
                parser.add_option("-i", "--iterations", type=int, dest="iters", help="Number of iterations of the test to run", default=3)
                #parser.add_option("-p", "--parallel", type=int, dest="parallel", help="Number of  tests to run in parallel - Unimplimented", default=1)
                parser.add_option("-s", "--servers", dest="servers", help="Host name of servers to test.", default=None)
	...
	def main(self):
		...
		for server in self.servers:
			...
			try:
				...
				sys.stderr.write("Could not open connection to  %s: %s\n" %(server,  str(e)))	
				sys.stderr.write("Skipping remaining tests for %s\n" %(server))	
